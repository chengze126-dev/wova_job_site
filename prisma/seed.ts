...
import { extraClients, marketplaceJobs } from "./marketplace-data";
import { CODING_QUESTIONS, toCodeQuestionRow } from "../lib/coding-questions";
import { DEMO_EXTRAS, stringifyExtras } from "../lib/profile-extras";
...
const questions = [
  ...
  {
    category: "Time",
    prompt: "You estimated 10 hours and you are at 9 hours with ~40% left. What now?",
    options: [
...
async function main() {
  ...
  const pair = talent.id < client.id ? { userAId: talent.id, userBId: client.id } : { userAId: client.id, userBId: talent.id };
  ...
  console.log(`Users: ${admin.email}, ${client.email}, ${client2.email}, ${talent.email}, ${talent2.email}`);
}
